                ...
                <div class="flex flex-col rounded-2xl">
                        ...
                        <div class="bg-mines flex flex-col gap-4 justify-center items-center py-4 pb-4 w-full rounded-t-xl">
                            <div class="flex items-stretch max-md:flex-col-reverse gap-4 max-w-[850px] w-full mx-auto">
                                <div class="flex justify-center items-center h-full">
                                    <div class="md:flex-col flex relative gap-2 h-full">
                                        ...
                                        <!-- Multiplier steps -->
                                        <div v-for="step in visibleMultiplierSteps" :key="step.gems" :class="[
                                        ...
                                        <!-- Down Arrow -->
                                        <div v-if="canScrollDown" @click="scrollMultipliersDown" class="absolute z-[1000] bottom-0 left-1/2 transform -translate-x-1/2 cursor-pointer text-white/70 hover:text-white transition-colors rounded-full p-1">
                                ...
                                <div class="grid flex-1 grid-cols-5 gap-2">
                                    <div v-for="(cell, index) in mineStore.cells" :key="index" :class="[
                                        'field',
                                        {
                            ...
                            <div class="bg-secondary-bg/80 max-w-[850px] border-secondary-bg/50 flex gap-3 items-center px-4 py-3 w-full rounded-2xl border">
                                <div class="main-input-small !bg-secondary-sidebar-dark/50 flex gap-1">
                                    ...
                                    <div class="bg-primary/10 text-primary hover:bg-primary/20 px-2 py-1.5 text-sm leading-none rounded-lg cursor-pointer" @click="setBetDouble" v-if="!mineStore.isGameActive">
                                        2X
                                    </div>
                                </div>
                                ...
                                <div class="main-input-small justify-between !bg-secondary-sidebar-dark/50 flex gap-1 relative">
                                    ...
                                    <div v-if="showMinesDropdown" class="bg-secondary-sidebar border-secondary-bg overflow-y-auto absolute right-0 left-0 top-full z-10 mt-1 max-h-40 rounded-lg border">
                                        <div v-for="option in minesOptions" :key="option" class="hover:bg-secondary-bg px-3 py-2 cursor-pointer" @click="selectedMinesCount = option; showMinesDropdown = false; playClickSound();">
                                            {{ option }}
                                        </div>
                                    </div>
                                </div>

                                <button v-if="mineStore.isGameIdle" @click="startGame" :disabled="!canStartGame || mineStore.loading" class="btn btn-primary w-fit disabled:opacity-50 disabled:cursor-not-allowed flex flex-shrink-0 justify-center items-center px-10">
                                    {{ mineStore.loading ? 'Loading...' : 'Bet' }}
                                </button>

                                <button v-else-if="mineStore.canCashout && !mineStore.gameOver" @click="handleCashout" :disabled="mineStore.loading" class="btn btn-primary w-fit disabled:opacity-50 flex flex-shrink-0 justify-center items-center px-10">
                                    Cashout {{ (mineStore.betAmount * mineStore.multiplier).toFixed(2) }}
                                </button>

                                <button v-else-if="mineStore.gameOver" @click="resetGame" class="btn btn-primary w-fit flex flex-shrink-0 justify-center items-center px-10">
                                    New Game
                                </button>
                    ...
                    <div class="bg-secondary-sidebar-dark border-blue_dark flex justify-between items-center p-6 rounded-b-xl border-t">
                        ...
                        <div class="bg-blue_light/5 flex gap-2 items-center py-1 pr-4 pl-1 rounded-full">
                            ...
                            <span class="text-blue_light font-bold transition-all duration-300" :class="{ 'text-green-400': previousBalance !== userStore.user?.balance }">
